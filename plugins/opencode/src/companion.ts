...
function runReview(argv: readonly string[]): number {
  try {
...
function runReviewForeground(options: ForegroundOptions): number {
...
function runReviewTracked(options: TrackedOptions): number {
  const id = newJobId();
  ...
  const opencodeOptions: { prompt: string; cwd: string; model?: string } = {
